...
std::string replaceYamlNode(std::string html, YAML::Node replacement)
{
  try
  {
    ...
    for (YAML::const_iterator it = replacement.begin(); it != replacement.end(); ++it)
    {
      boost::regex expr{"\\{" + it->first.as<std::string>() + "\\}"};
      copy = boost::regex_replace(copy, expr, it->second.as<std::string>().c_str());
    }
  ...
  try
  {
    ...
    for (YAML::const_iterator it = node.begin(); it != node.end(); ++it)
    {
      ...
      case YAML::NodeType::Scalar:
        ...
        copy = boost::regex_replace(copy, boost::regex{key.c_str()}, val.c_str());
        break;
      }
    }
...
std::string roles(std::string html, YAML::Node node)
{
  try
  {
    ...
    copy = boost::regex_replace(copy, current_role, node["roles"][0].as<std::string>());
    ...
    return boost::regex_replace(copy, previous_roles_regex, replacement.c_str());
  }
  catch (exception &e)
  {
    std::cerr << e.what() << std::endl;
...
std::string pages(std::string section, YAML::Node node)
{
  try
  {
    ...
    for (int count = 0; count < pages.size(); count++)
    {
      ...
      for (YAML::iterator job_it = jobs.begin(); job_it != jobs.end(); ++job_it)
      {
        YAML::Node job = *job_it;
        std::string jobSection = match[1];
        boost::smatch roleMatch;

        if (boost::regex_search(jobSection, roleMatch, boost::regex{"<roles>(.*?)</roles>"}) == false)
        {
        ...
        for (YAML::iterator role_it = roles.begin(); role_it != roles.end(); ++role_it)
        {
          YAML::Node role = *role_it;
          std::string roleSection = roleMatch[1];
          boost::smatch detailsMatch;

          if (boost::regex_search(jobSection, detailsMatch, boost::regex{"<details>(.*?)</details>"}) == false)
          {
          ...
          for (YAML::iterator details_it = details.begin(); details_it != details.end(); ++details_it)
          {
            YAML::Node detail = *details_it;
            detailsReplacement += replaceLineItem(detailsMatch[1], detail.as<std::string>());
          }

          roleSection = boost::regex_replace(roleSection, boost::regex{"<details>(.*?)</details>"}, detailsReplacement.c_str());
          roleSection = replaceScalars(roleSection, role);
          rolesReplacement += roleSection;
        }

        jobSection = boost::regex_replace(jobSection, boost::regex{"<roles>(.*?)</roles>"}, rolesReplacement.c_str());
        jobSection = replaceScalars(jobSection, job);
        replacement += jobSection;
      }

      result = boost::regex_replace(result, boost::regex{"<page" + X + ">(.*?)</page" + X + ">"}, replacement.c_str());
    }
    return result;
  }
  catch (exception &e)
